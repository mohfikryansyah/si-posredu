<x-app-layout>
    ...
    <div class="w-full p-5 bg-white mt-4 rounded-md shadow-sm">
        ...
        <h2 class="font-semibold md:text-lg text-md text-gray-600 leading-tight text-center">
            {{ $lansia->lansia->tempat_tanggal_lahir }}
        </h2>
    </div>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white mt-4">
        <table id="table_pemeriksaan_lansia">
            ...
            <tbody>
                @foreach ($allPemeriksaanLansiaSaatIni as $item)
                    <tr
                        ...
                        <td>
                            <div class="flex items-center">
                                <a href="javascript:void(0);" x-data data-id="{{ $item->id }}"
                                    data-lansia-id="{{ $item->lansia->id }}" data-employee-id="{{ $item->employee_id }}"
                                ...
                                <a data-id={{ $item->id }} data-nama="{{ $item->lansia->nama }}"
        ...
        <script>
            $(document).ready(function() {
                ...
                $('table').on('click', '.editbtn', function() {
                    var id = $(this).data('id');
                    var lansia_id = $(this).data('lansia-id');
                    ...
                    $('#edit_id').val(id);
                    $('#edit_lansia_id').val(lansia_id).trigger('change');
