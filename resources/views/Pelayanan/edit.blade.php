    ...
    <div class="md:grid grid-cols-3 gap-5">
        ...
        <div class="col-sm-auto">
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                <h1 class="bg-orange-400 px-5 py-2.5 text-gray-100 font-medium">Ubah Jadwal Pelayanan</h1>
                <div class="p-5">
                    <form action="{{ route('jadwal-pelayanan.update', ['jadwal_pelayanan' => $p->id]) }}" method="POST" class="max-w-sm mx-auto">
                        @method("PUT")
                        @csrf
                        <div class="mb-5">
                            <label for="nama"
                            ...
                            <x-text-input name="nama" id="nama"
                                class="{{ $errors->edit_pelayanan->has('nama') ? 'border-red-500' : 'border-gray-300' }}" value="{{ old('nama', $p->nama) }}"></x-text-input>
                            @error('nama', 'edit_pelayanan')
                        ...
                        <div class="mb-5">
                            ...
                            <x-date-input name="tanggal_pelayanan" id="tanggal_pelayanan"
                                class="{{ $errors->edit_pelayanan->has('tanggal_pelayanan') ? 'border-red-500' : 'border-gray-300' }}"
                                autocomplete="off" value="{{ old('tanggal_pelayanan', $p->tanggal_pelayanan) }}"></x-date-input>
                            @error('tanggal_pelayanan', 'edit_pelayanan')
                                <p class="text-red-500 text-xs">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-5">
                            ...
                            <textarea rows="4"
                                class="{{ $errors->edit_pelayanan->has('deskripsi') ? 'border-red-500' : 'border-gray-300' }} bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                name="deskripsi" id="deskripsi" placeholder="Deskripsi singkat tentang layanan">{{ old('deskripsi', $p->deskripsi) }}</textarea>
                            @error('deskripsi', 'edit_pelayanan')
                                <p class="text-red-500 text-xs">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="time" id="start" name="start"
                                        ...
                                        value="{{ old('start', $p->start) }}" required />
                                    @error('start', 'edit_pelayanan')
                                        <p class="text-red-500 text-xs">{{ $message }}</p>
                                    @enderror
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="time" id="end" name="end"
                                        class="bg-gray-50 border leading-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        value="{{ old('end', $p->end) }}" required />
                                    @error('end', 'edit_pelayanan')
                                        <p class="text-red-500 text-xs">{{ $message }}</p>
                                    @enderror
                                </div>
                            </div>
