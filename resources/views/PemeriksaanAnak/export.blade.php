                ...
                <div class="col-span-2">
                    ...
                    <select id="export_anak_id" name="anak_id"
                        ...
                        @foreach ($pemeriksaans as $pemeriksaan)
                            @if (old('anak_id') === $pemeriksaan->anak->id)
                                <option unvalue="{{ $pemeriksaan->anak->id }}" selected>{{ $pemeriksaan->anak->nama }}</option>
                            @else
                                <option value="{{ $pemeriksaan->anak->id }}">{{ $pemeriksaan->anak->nama }}</option>
                            @endif
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-1 col-span-2">
                    <label for="start"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mulai tanggal<span
                            class="text-red-500">*</span></label>
                    <x-date-input name="start" id="start" class="{{ $errors->export_pemeriksaan_anak->has('start') ? 'border-red-500' : 'border-gray-300' }}" value="{{ old('start') }}"></x-date-input>
                ...
                <div class="md:col-span-1 col-span-2">
                    <label for="end"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Sampai tanggal<span
                            class="text-red-500">*</span></label>
                    <x-date-input name="end" id="end" class="{{ $errors->export_pemeriksaan_anak->has('end') ? 'border-red-500' : 'border-gray-300' }}" value="{{ old('end') }}"></x-date-input>
