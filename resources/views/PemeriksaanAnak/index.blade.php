        ...
        <table id="table_pemeriksaan_anak">
            <thead>
                <tr>
                    ...
                    <th scope="col">Status Imunisasi</th>
                ...
                @foreach ($pemeriksaans as $pemeriksaan)
                    <tr
                        ...
                        <th class="whitespace-nowrap" title="{{ $pemeriksaan->anak->nama }}">
                            {{ Str::limit($pemeriksaan->anak->nama, 21) }}
                        </th>
                        <td class="whitespace-nowrap md:whitespace-normal"
                            title="{{ $pemeriksaan->tanggal_pemeriksaan }}">
                            {{ Str::limit($pemeriksaan->tanggal_pemeriksaan, 21) }}
                        </td>
                        <td class="whitespace-nowrap md:whitespace-normal" title="{{ $pemeriksaan->berat_badan }}">
                            {{ $pemeriksaan->berat_badan . ' kg' }}
                        </td>
                        <td class="whitespace-nowrap md:whitespace-normal" title="{{ $pemeriksaan->tinggi_badan }}">
                            {{ Str::limit($pemeriksaan->tinggi_badan, 15) . ' cm' }}
                        </td>
                        <td class="whitespace-nowrap md:whitespace-normal" title="{{ $pemeriksaan->tekanan_darah }}">
                            {{ Str::limit($pemeriksaan->tekanan_darah, 15) . ' mmHg' }}
                        </td>
                        <td class="whitespace-nowrap md:whitespace-normal" title="{{ $pemeriksaan->suhu_tubuh }}">
                            {{ Str::limit($pemeriksaan->suhu_tubuh, 15) . '°C' }}
                        </td>
                        <td class="whitespace-nowrap md:whitespace-normal"
                            title="{{ $pemeriksaan->status_imunisasi }}">
                            {{ Str::limit($pemeriksaan->status_imunisasi, 13, ' ') }}
                        </td>
                        <td class="whitespace-nowrap md:whitespace-normal"
                            title="{{ $pemeriksaan->riwayat_penyakit }}">
                            {{ Str::limit($pemeriksaan->riwayat_penyakit, 13, ' ') }}
                        </td>
                        <td class="whitespace-nowrap md:whitespace-normal" title="{{ $pemeriksaan->catatan }}">
