@props(['value' => false, 'categories', 'category' => 'Semua Kategori'])

<form action="{{ route('blog.search') }}" method="GET" class="max-w-lg mx-auto">
    <div class="flex">
        <input type="hidden" id="category" name="category" value="{{ $category }}"> <!-- Input hidden untuk kategori -->
        ...
        <div id="dropdown"
            ...
            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">
                <li>
                    <button type="button"
                        class="category-option inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                        data-category="Semua Kategori">Semua Kategori</button>
                </li>
        ...
        <div class="relative w-full">
            <input type="search" id="search-dropdown" name="keyword" value="{{ $value }}"
...
@push('search')
    <script>
        // Update kategori berdasarkan pilihan dropdown
        document.querySelectorAll('.category-option').forEach(function(button) {
            button.addEventListener('click', function() {
                const category = this.getAttribute('data-category');
                document.getElementById('category').value = category;
                document.getElementById('dropdown-button').textContent = category;
            });
        });
    </script>
@endpush
