...
<x-modal name="export_pemeriksaan_remaja" id="export_pemeriksaan_remaja" :show="false" maxWidth="2xl" focusable>
    <div class="p-6">
        ...
        <form action="{{ route('pemeriksaanRemaja.export') }}" method="GET" class="mt-4">
            @csrf
            <div class="grid gap-4 mb-4 grid-cols-2">
                <div class="col-span-2">
                    ...
                    <select id="export_remaja_id" name="remaja_id"
                        ...
                        @foreach ($pemeriksaans as $remaja)
                            @if (old('remaja_id') === $remaja->remaja->id)
                                <option value="{{ $remaja->remaja->id }}" selected>{{ $remaja->remaja->nama }}</option>
                            @else
                                <option value="{{ $remaja->remaja->id }}">{{ $remaja->remaja->nama }}</option>
                            @endif
                        @endforeach
                    </select>
                    @error('remaja_id', 'export_pemeriksaan_remaja')
                ...
                <div class="md:col-span-1 col-span-2">
                    <label for="start"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mulai tanggal<span
                            class="text-red-500">*</span></label>
                    <x-date-input name="start" id="start" class="{{ $errors->export_pemeriksaan_remaja->has('start') ? 'border-red-500' : 'border-gray-300' }}" value="{{ old('start') }}"></x-date-input>
                ...
                <div class="md:col-span-1 col-span-2">
                    <label for="end"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Sampai tanggal<span
                            class="text-red-500">*</span></label>
                    <x-date-input name="end" id="end" class="{{ $errors->export_pemeriksaan_remaja->has('end') ? 'border-red-500' : 'border-gray-300' }}" value="{{ old('end') }}"></x-date-input>
